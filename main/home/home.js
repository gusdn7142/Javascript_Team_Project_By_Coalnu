...
document.addEventListener("DOMContentLoaded", function () {
  function setupNavEventListeners() {
    ...
    navTags.forEach((navTag) =>
      navTag.addEventListener("click", (event) => getBookByNavTag(event))
    );
...
categorys.forEach((category) =>
  category.addEventListener("click", (event) => getBookByCategory(event))
);
dropdown.forEach((dropdown) =>
  dropdown.addEventListener("click", (event) => getBookByCategory(event))
);
...
const getBookByNavTag = async (event) => {
  ...
  try {
    ...
    const response = await fetch(
      `${bootAPIDomain}?key=${apiKey}&apiType=json&detailSearch=true&category=${navTagName}&pageNum=${pageNum}&pageSize=${pageSize}`
    );
  ...
  //4-3) View 렌더링
  bookTotalCount.textContent = bookDataList.total.toLocaleString(); //도서 총 개수 셋팅

  const imageAndTextHTML = bootImageAndTextList
    .map(
      //도서 목록 표시
      (bootImageAndText) => {
        let bootTitle = "";
        ...
        return `<div  class="book-ImageAndText-one" >          
            ...
            <div class="book-image  custom-mg-bottom-8" > 
                <a href="../detail/detail.html?titleKeyword=${
                  bootImageAndText.titleInfo
                }&authorKeyword=${bootImageAndText.authorInfo}" >   
                    <img src="${
                      bootDetailImageDomain + bootImageAndText.imageUrl
  ...
  document.getElementById("book-ImageAndText-List").innerHTML =
    imageAndTextHTML;
};
...
const getBookByCategory = async (event) => {
  ...
  let url = "";
  if (categoryName === "전체") {
    //이슈 : URL에 category=currentNavTagName 를 넣을수 있는 방법을 찾아야 함.
    url = new URL(`
  ...
  const categoryActiveElements = document.querySelectorAll(
    ".categorys-btn.categorys-btn-active"
  ); //기존 categorys-btn-active 삭제
