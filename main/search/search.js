//변수 설정
const apikey =
  "76faa9053c59326364cd62f1f1375e2d77db3e92ae9879832c4d569414929619"; //config.apikey; //api_key
let bookList = [];
...
//api주소 설정
let url = new URL(
  `https://www.nl.go.kr/NL/search/openApi/search.do?key=${apikey}&apiType=json&detailSearch=true`
);
...
const searchBook = async () => {
  ...
  url = new URL(
    `https://www.nl.go.kr/NL/search/openApi/search.do?key=${apikey}&apiType=json&detailSearch=true&v1=${keyword}&f1=title`
  );
  await getBookInfo();
};
...
const render = () => {
  ...
  const bookItemsHTML = displayedBooks
    .map((book) => {
      let bookTitle = "";
      let bookAuthor = "";
      let bookShowTitle = "";
      ...
      if (book.titleInfo == "") {
        ...
        bookTitle =
          bookShowTitle.length > 10
            ? bookShowTitle.slice(0, 10) + "..."
            : bookShowTitle;
      }
      ...
      if (book.authorInfo == "") {
        ...
        bookAuthor =
          bookShowAuthor.length > 10
            ? bookShowAuthor.slice(0, 10) + "..."
            : bookShowAuthor;
      }
...
const paginationRender = () => {
  ...
  if (pageNum > 1) {
    paginationHTML = `
        ...
        <li class="main-bottom-pagination-link" onclick="moveToPage(${
          pageNum - 1
        })">
          <a class="custom-fs-16 responsive-fs-12 medium-text">&lt;</a>
        </li>`;
  ...
  for (let i = firstPage; i <= lastPage; i++) {
    paginationHTML += `
      <li class="main-bottom-pagination-link ${
        i === pageNum ? "active" : ""
